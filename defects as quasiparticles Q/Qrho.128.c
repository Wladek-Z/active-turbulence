...
double flowparameter[Lx][Ly][Lz],Qflowparameter[Lx][Ly][Lz];
...
double Abulk = 0.0; //0.0; /*determines bulk liquid crystal free energy*/
...
/* work out antisymmetric part of the stress tensor, this will go to the bodyforce */
		...
		+0.0*phivr*(DG2yz[i][j][k]-DGzy)/2.0;
	      ...
	      Qflowparameter[i][j][k]=0.5*(omega*omega-DD*DD);
...
fprintf(output1, "%d %d %d %E %E %E %E %E %E %E %E %E %E %E %E %E %E %E %E %E %E\n",
				  i, j, k,
				  ...
				  defectcharge[i][j][k],
				  density[i][j][k],
	                          phi[i][j][k],flowparameter[i][j][k],Qflowparameter[i][j][k],compressibility[i][j][k]);
			}
			fprintf(output1, "\n");
		}
		fprintf(output1, "\n");
